 ...
 */
UCLASS()
class CRAFTOR_API UGoodsFunctionLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static FORCEINLINE T* PickOneFromWeightedList(FRandomStream& RandStream, TArray<T>& WeightedItems, const float TotalWeight = -1.0f)
	{
		...
		//UE_LOG(LogCTGame, Log, TEXT("PickOne total weight: %f  picked weight %f"), TotalWeightedChance, PickedWeight);
		...
		for (T& WeightedItem : WeightedItems)
		{
			...
			{
				//UE_LOG(LogCTGame, Log, TEXT("Picked: %s"), *WeightedItem.UniqueNameBase.ToString())
				return &WeightedItem;
			}
		}
		return nullptr;
